...
foreach ($TEC as $entry) {
    ...
    $candidate   = $entry['candidate'];
    $choices     = $entry['choices'];   // candidate's choice list order
    $quotaList   = $entry['quota'];     // {CFF=>true|false,...}
    $techMerit   = $candidate['technical_merit_position'] ?? [];

    $assigned = false;

    // Loop through choices in candidate's priority order
    foreach ($choices as $cadreShort) {

        // Resolve Technical Cadre Short → Cadre Code
        $cadreCode = $cadres['TECHNICAL'][$cadreShort]['code'] ?? null;
        if (!$cadreCode) continue;

        // Check if candidate has technical merit for this cadre
        if (!isset($techMerit[$cadreShort])) continue;

        /*
        STEP 1 — MERIT FIRST (MQ)
        */
        if ($tech_post_remaining[$cadreCode]['MQ'] > 0) {
            ...
            $technical_placement_result[] = [
                ...
                'cadre'     => $cadreShort,
                'quota'     => 'MERIT',
                'tech_merit_position' => $techMerit[$cadreShort]
            ];
            break; // stop processing candidate
        }

        /*
        STEP 2 — QUOTA (CFF → EM → PHC)
        */
        foreach (['CFF', 'EM', 'PHC'] as $quotaType) {

            if (empty($quotaList[$quotaType])) continue;
        ...
        // Try next choice if no seat assigned
    }
...
/*
-------------------------------------------------------------
STEP 4 — SORT ASSIGNED CANDIDATES WITHIN EACH TECHNICAL CADRE
-------------------------------------------------------------
*/
$final_technical_result = [];
$cadre_groups = [];

foreach ($technical_placement_result as $entry) {
    if ($entry['cadre'] === null) {
        $final_technical_result[] = $entry; // keep NOT_ASSIGNED as-is
        continue;
    }
    $cadre_groups[$entry['cadre']][] = $entry;
}

// Sort each cadre by tech_merit_position ascending
foreach ($cadre_groups as $cadreShort => $group) {
    usort($group, function($a, $b) {
        return $a['tech_merit_position'] <=> $b['tech_merit_position'];
    });
    $final_technical_result = array_merge($final_technical_result, $group);
}

// Optional: print final results
echo '<h2>Technical Placement</h2><br>';
echo '<pre>';
print_r($final_technical_result);
echo '</pre>';
